...
SCORE_THRESHOLD = 20  # Products with total score >= 20 get made today
...
def pick_todays_products(scored):
    picks = [
        p for p in scored
        if p.get("make_today") or p.get("total", 0) >= SCORE_THRESHOLD
    ]

    # Safety fallback if nothing qualifies
    if len(picks) == 0:
        picks = sorted(
            scored,
