...
class ProductFileBuilder:
    ...
    def build_budget_tracker(self, theme):
        ...
        for row in range(2, 102):
            ...
            ws[f"D{row}"] = 0
            ws[f"E{row}"] = 0
            ws[f"F{row}"] = "=D2-E2" if row == 2 else f"=F{row-1}+D{row}-E{row}"

        for col, width in {"A": 15, "B": 30, "C": 20, "D": 15, "E": 15, "F": 18}.items():
            ws.column_dimensions[col].width = width

        dashboard = wb.create_sheet("Dashboard")
        dashboard["A1"] = f"Budget Tracker Dashboard - {theme['name']}"
        dashboard["A1"].font = Font(bold=True, size=18, color=theme["dark"])
        dashboard["A1"].fill = PatternFill("solid", fgColor=theme["accent"])
        ...
        categories["A1"].font = Font(bold=True)

        category_list = [
            "Income", "Salary", "Rent", "Groceries", "Utilities",
            "Insurance", "Transportation", "Fuel", "Entertainment",
